...
MC_LIST="Newsletter de Le crochet d'argent"
...
ART_SUBJECT="Nouvel article: $ART_TITLE"
...
if [ "$ANSWER" = "y" ]; then
  ./chimper-fr.rb -l "$MC_LIST" -t "$ART_TITLE" -s "$ART_SUBJECT" -f "$ART_FEATURED_IMAGE" --template new-fr --message-file /tmp/article.$$ --link "$URL"
fi
