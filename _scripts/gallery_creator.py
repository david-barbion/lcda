...
output_file = join("_data/galleries/", image_path + ".yml")
input_file = output_file
extensions= ['jpg', 'png']

# set correct path
path = join("assets/resized/", image_path)
...
for f in files:
    if f[f.rfind('-')+1:f.rfind('.')] != 'thumbnail':
        ...
        print('  rename %s to %s' % (join(path, f),join(path, newf)))
...
for image_set in new_gallery:
    ...
    for image in new_gallery[image_set]:
        ...
        print("  image: %s w:%s h:%s") % (image, width, height)
